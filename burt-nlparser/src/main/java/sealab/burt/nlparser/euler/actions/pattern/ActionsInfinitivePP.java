...
public class ActionsInfinitivePP extends NLActionPatternParser {

    Set<ImmutablePair<String, String>> clauseSeparators = JavaUtils.getPairSet("->");

    public ActionsInfinitivePP(boolean debugEnabled) {
        super(debugEnabled);
    }
    ...
    private List<NLAction> getActions(SemanticGraph dependencies, IndexedWord verbToken, Sentence sentence) {
        ...
        // is it a valid verb?
        if ((!verbToken.tag().equals("VB") && !verbToken.tag().equals("VBP"))
                && (!TextProcessor.checkGeneralPos(verbToken.tag(), "NN")
                || !SentenceUtils.stringEqualsToAnyToken(SentenceUtils.UNDETECTED_VERBS, verbToken.lemma()))) {
        ...
        if (objRelation != null) {
            ...
            // probably there are other verbs that need this check
            if (!verbToken.lemma().equals("go") && !verbToken.lemma().equals("click")
                    && !verbToken.lemma().equals("tap") && !verbToken.lemma().equals("scroll") && !verbToken.lemma().equals("swipe")
                    && !verbToken.lemma().equals("check")) {
                return actions;
            }
